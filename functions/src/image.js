...
function imageFromHTML(orgHTML) {
	...
	const dom = new JSDOM(
		`<!DOCTYPE html><head><style>${graphStyle}</style></head><body><div class="graph" id="graph">${orgHTML}</div><p id="data"></p><script>${domToImageScript}</script><script>${domScript}</script></body>`,
		{runScripts: 'dangerously', pretendToBeVisual: true}
	)

	const svgData = dom.window.document.getElementById('data').innerHTML

	while (svgData.length < 2) {
		setTimeout(() => {
			svgData = dom.window.document.getElementById('data').innerHTML
		}, 200)
	}
